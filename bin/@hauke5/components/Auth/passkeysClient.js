import { base64ToBytes, bytesToBase64, Log } from "@hauke5/lib/utils";
import { finalizeSignup, initAuth, initSignup, verifyAuth } from "./passkeysServer";
 ...
 * @returns
 */
export async function signUp(username, openDialog) {
    try {
        ...
        const challenge = publicKey.challenge = Uint8Array.from(publicKey.challenge);
        publicKey.user.id = Uint8Array.from(init.publicKey.user.id);
        const user = {
            username,
            id: bytesToBase64(publicKey.user.id),
        ...
        user.email = details.items.Email.value;
        user.displayName = details.items.DisplayName.value;
...
export async function login(username) {
    try {
        ...
        const allowCredentials = passIds.map(cr => ({
            id: base64ToBytes(cr),
            type: "public-key"
        }));
        ...
        const challenge = Uint8Array.from(init.challenge);
        const options = {
            publicKey: {
                challenge,
                allowCredentials,
        ...
        const rawCreds = await navigator.credentials.get(options);
        if (!rawCreds) {
