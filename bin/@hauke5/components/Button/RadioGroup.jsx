...
export function RadioGroup(props) {
    const [selected, setSelected] = useState(getSelectedArray(props.children.length, props.selectedIndex));
    useEffect(() => {
    ...
    return <div className={`${styles.group} ${props.className ?? ''}`} style={style}>
      {props.children.map((c, i) => <div className={`${styles.item} ${selected[i] ? styles.selected : ''}`} key={i} onClick={e => select(i)}>{c}</div>)}
   </div>;
    function getSelectedArray(length, selectedIndex) {
        const index = selectedIndex ?? 0;
        return Array(length).fill(false)
            .map((_, i) => typeof index === 'number'
            ? i === index ? true : false
            : index[i] ? true : false);
    }
}
