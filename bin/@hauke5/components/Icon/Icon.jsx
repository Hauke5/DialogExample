 ...
 * @returns
 */
export function Icon({ children, mdi, className = '', size = 25, pre = false, onMouseUp, onClick, ...props }) {
    const clicked = (e) => {
        e.preventDefault();
        if (onMouseUp)
            onMouseUp(e);
    ...
    return <div className={`${styles.icon} ${className}`} style={{ minWidth: `${size}px` }} {...props} onMouseUp={onMouseUp ? clicked : undefined} onClick={onClick ? clicked : undefined}>
